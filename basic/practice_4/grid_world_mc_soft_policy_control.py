...
class SoftPolicyMonteCarloControl:
    ...
    # 탐욕적인 정책을 생성함
    def generate_soft_greedy_policy(self):
        new_policy = dict()
                ...
                if (i, j) in TERMINAL_STATES:
                    ...
                    for action in range(self.env.action_space.NUM_ACTIONS):
                        if action in max_prob_actions:
                            action_probs.append(
                                (1 - EPSILON) / len(max_prob_actions) + EPSILON / self.env.action_space.NUM_ACTIONS
                            )
                        else:
                            action_probs.append(
                                EPSILON / self.env.action_space.NUM_ACTIONS
                            )
    ...
    def soft_policy_control(self):
        ...
        while iter_num < self.max_iteration:
            ...
            _, error = self.generate_soft_greedy_policy()
